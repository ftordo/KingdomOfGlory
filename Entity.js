var Entity = Class.extend(function(){
  ...
  this.getCenterX = function(){ return this.x + (this.width * 0.5);} // this.width >> 1; é mais eficiente
  ...
  this.render=function(ctx){
	    if(!this.active ||ctx===null || ctx===undefined || !this.visible) return;
  ...
  this.hitTestRectangle = function(otherEntity){
	 ...
	 // verificar se ha colisão no eixo X
	  if(Math.abs(vx) < combinedHalfWidths){
		...
		if(Math.abs(vy) < combinedHalfHeights){hit = true;} //Existe mesmo uma colisão  
		else{hit = false;}//Não há colisão no eixo Y    
	 }else {hit = false;}//Não há colisão no eixo X
